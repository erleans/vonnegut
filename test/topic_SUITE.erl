...
index_bug(Config) ->
    ...
    {ok, #{Topic := #{0 := #{record_set := Reply}}}} =
               vg_client:fetch(Topic, 0),
...
limit(Config) ->
    ...
    {ok, #{Topic := #{0 := #{record_set := Reply2}}}} =
        vg_client:fetch(Topic, 0, 1000),
    ?assertEqual(24, length(Reply2)),

    {ok, #{Topic := #{0 := #{record_set := []}}}} =
               vg_client:fetch(Topic, 0, 1),
